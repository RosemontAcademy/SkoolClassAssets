/**
 * rename_sprites.mjs
 * Renames Pokémon Showdown sprite GIFs from pure numbers to {num}-{name}.gif
 *
 * Usage:
 *   node rename_sprites.mjs          ← dry run (shows what would change)
 *   node rename_sprites.mjs --apply  ← actually renames the files
 */
...
const DEX = {
  ...
  665:'spewpa',666:'vivillon',667:'litleo',668:'pyroar',669:'flabebe',
  ...
  865:'sirfetchd',866:'mr-rime',867:'runerigus',868:'milcery',869:'alcremie',
  ...
  981:'farigiraf',982:'dudunsparce',983:'kingambit',984:'great-tusk',
  985:'scream-tail',986:'brute-bonnet',987:'flutter-mane',988:'slither-wing',
  989:'sandy-shocks',990:'iron-treads',991:'iron-bundle',992:'iron-hands',
  993:'iron-jugulis',994:'iron-moth',995:'iron-thorns',996:'frigibax',
  997:'arctibax',998:'baxcalibur',999:'gimmighoul',1000:'gholdengo',
  1001:'wo-chien',1002:'chien-pao',1003:'ting-lu',1004:'chi-yu',
  1005:'roaring-moon',1006:'iron-valiant',1007:'koraidon',1008:'miraidon',
  1009:'walking-wake',1010:'iron-leaves',1011:'dipplin',1012:'poltchageist',
  1013:'sinistcha',1014:'okidogi',1015:'munkidori',1016:'fezandipiti',
  1017:'ogerpon',1018:'archaludon',1019:'hydrapple',1020:'gouging-fire',
  1021:'raging-bolt',1022:'iron-boulder',1023:'iron-crown',1024:'terapagos',
  1025:'pecharunt',
};

// ── Extended form IDs (10001+) — fetched from PokéAPI ────────────────────────
// These will be resolved at runtime via API. Known entries are seeded here
// so we can run fully offline if needed.
const FORM_NAMES_SEED = {
  10001:'deoxys-attack',10002:'deoxys-defense',10003:'deoxys-speed',
  10004:'wormadam-sandy',10005:'wormadam-trash',10006:'shaymin-sky',
  10007:'giratina-origin',10008:'rotom-heat',10009:'rotom-wash',
  10010:'rotom-frost',10011:'rotom-fan',10012:'rotom-mow',
  10013:'castform-sunny',10014:'castform-rainy',10015:'castform-snowy',
  10016:'basculin-blue-striped',10017:'darmanitan-zen',10018:'meloetta-pirouette',
  10019:'tornadus-therian',10020:'thundurus-therian',10021:'landorus-therian',
  10022:'kyurem-black',10023:'kyurem-white',10024:'keldeo-resolute',
  10025:'meowstic-f',10026:'aegislash-blade',
  10027:'pumpkaboo-small',10028:'pumpkaboo-large',10029:'pumpkaboo-super',
  10030:'gourgeist-small',10031:'gourgeist-large',10032:'gourgeist-super',
  10033:'venusaur-mega',10034:'charizard-mega-x',10035:'charizard-mega-y',
  10036:'blastoise-mega',10037:'alakazam-mega',10038:'gengar-mega',
  10039:'kangaskhan-mega',10040:'pinsir-mega',10041:'gyarados-mega',
  10042:'aerodactyl-mega',10043:'mewtwo-mega-x',10044:'mewtwo-mega-y',
  10045:'ampharos-mega',10046:'scizor-mega',10047:'heracross-mega',
  10048:'houndoom-mega',10049:'tyranitar-mega',10050:'blaziken-mega',
  10051:'gardevoir-mega',10052:'mawile-mega',10053:'aggron-mega',
  10054:'medicham-mega',10055:'manectric-mega',10056:'banette-mega',
  10057:'absol-mega',10058:'garchomp-mega',10059:'lucario-mega',
  10060:'abomasnow-mega',10061:'floette-eternal',
  10062:'latias-mega',10063:'latios-mega',
  10064:'swampert-mega',10065:'sceptile-mega',10066:'sableye-mega',
  10067:'altaria-mega',10068:'gallade-mega',10069:'audino-mega',
  10070:'sharpedo-mega',10071:'slowbro-mega',10072:'steelix-mega',
  10073:'pidgeot-mega',10074:'glalie-mega',10075:'diancie-mega',
  10076:'metagross-mega',10077:'kyogre-primal',10078:'groudon-primal',
  10079:'rayquaza-mega',
  10080:'pikachu-rock-star',10081:'pikachu-belle',10082:'pikachu-pop-star',
  10083:'pikachu-phd',10084:'pikachu-libre',10085:'pikachu-original',
  10086:'hoopa-unbound',10087:'camerupt-mega',10088:'lopunny-mega',
  10089:'salamence-mega',10090:'beedrill-mega',
};

// ── Fetch missing form names from PokéAPI ────────────────────────────────────
async function resolveFormName(id) {
  if (FORM_NAMES_SEED[id]) return FORM_NAMES_SEED[id];
  try {
    const res = await fetch(`https://pokeapi.co/api/v2/pokemon/${id}`);
    if (!res.ok) return null;
    const data = await res.json();
    return data.name; // e.g. "charizard-mega-x"
  } catch {
    return null;
  }
}
...
// Separate pure-number files from files that already have text suffixes
// e.g. "855-antique.gif" has dexNum=855 and suffix="antique"
const toProcess = files.map(file => {
  const base = path.basename(file, '.gif');
  const firstDash = base.indexOf('-');
  const numStr = firstDash === -1 ? base : base.slice(0, firstDash);
  const suffix = firstDash === -1 ? '' : base.slice(firstDash + 1);
  const dexNum = parseInt(numStr, 10);
  return { file, dexNum, suffix };
}).filter(({ dexNum }) => !isNaN(dexNum));

// Collect unique IDs that need API lookup
const needsLookup = [...new Set(
  toProcess
    .filter(({ dexNum }) => dexNum >= 10001 && !FORM_NAMES_SEED[dexNum])
    .map(({ dexNum }) => dexNum)
)];

if (needsLookup.length > 0) {
  console.log(`Fetching ${needsLookup.length} unknown form names from PokéAPI...`);
  const CONCURRENCY = 10;
  for (let i = 0; i < needsLookup.length; i += CONCURRENCY) {
    const batch = needsLookup.slice(i, i + CONCURRENCY);
    const results = await Promise.all(batch.map(id => resolveFormName(id)));
    batch.forEach((id, idx) => {
      if (results[idx]) FORM_NAMES_SEED[id] = results[idx];
    });
    process.stdout.write(`  ${Math.min(i + CONCURRENCY, needsLookup.length)}/${needsLookup.length}\r`);
  }
  console.log('\nDone fetching.');
}
...
for (const { file, dexNum, suffix } of toProcess) {
  let pokeName;
  if (dexNum <= 1025) {
    pokeName = DEX[dexNum];
  } else {
    pokeName = FORM_NAMES_SEED[dexNum] ?? null;
  }

  if (!pokeName) {
    console.warn(`  ⚠  Unknown ID ${dexNum} — skipping ${file}`);
    unknown++;
    continue;
  }

  // Build new filename
  // For 10001+ files the pokeName already encodes the form (e.g. "charizard-mega-x")
  // For 1-1025 files with an existing suffix, insert the name before the suffix
  let newBase;
  if (suffix === '') {
    newBase = `${dexNum}-${pokeName}`;
  } else if (dexNum >= 10001) {
    // Name already contains form info; suffix is redundant — just append
    newBase = `${dexNum}-${pokeName}-${suffix}`;
  } else {
    newBase = `${dexNum}-${pokeName}-${suffix}`;
  }

  const newFile = `${newBase}.gif`;
  if (newFile === file) { skipped++; continue; }

  plan.push({ from: file, to: newFile, dexNum });
}

// Sort for readable output
plan.sort((a, b) => a.dexNum - b.dexNum);

if (DRY_RUN) {
  console.log(`\n=== DRY RUN — ${plan.length} files would be renamed ===\n`);
  // Show first 40 and last 10 as sample
  const show = plan.length <= 60 ? plan : [...plan.slice(0, 40), { from: '...', to: `(${plan.length - 50} more)`, dexNum: -1 }, ...plan.slice(-10)];
  for (const { from, to } of show) console.log(`  ${from.padEnd(45)} →  ${to}`);
  console.log(`\nSummary: ${plan.length} to rename, ${skipped} already correct, ${unknown} unknown/skipped`);
  console.log('\nRun with --apply to execute the renames.');
} else {
  console.log(`Renaming ${plan.length} files...`);
  let done = 0, failed = 0;
  const { setTimeout: wait } = await import('timers/promises');
  for (const { from, to } of plan) {
    const src = path.join(DIR, from);
    const dst = path.join(DIR, to);
    // Skip already-renamed files (partial run recovery)
    if (!fs.existsSync(src)) { skipped++; continue; }
    let tries = 0;
    while (tries < 5) {
      try { fs.renameSync(src, dst); break; }
      catch (e) {
        if (e.code === 'EBUSY' && tries < 4) { await wait(150); tries++; }
        else { console.warn(`  FAILED: ${from} — ${e.code}`); failed++; tries = 5; }
      }
    }
    if (tries < 5 || fs.existsSync(dst)) done++;
    if ((done + failed) % 100 === 0) process.stdout.write(`  ${done + failed}/${plan.length}\r`);
  }
  console.log(`\nDone! ${done} renamed, ${skipped} already correct, ${unknown} unknown, ${failed} failed.`);
}
